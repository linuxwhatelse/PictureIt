...
Spectrum::Spectrum(int spectrum_bar_count) : audio_ring_buf(RING_BUF_SIZE),
    ...
    ASPLIB_ERR asplib_error = this->vis_processor.Create(
        vis_processor_configurator, this->spectrum_bar_count, this->fft_frame_size);
...
void Spectrum::draw_spectrum() {
    ...
    glPushMatrix();
        ...
        if (this->audio_ring_buf.get_StoredSamples() >= this->fft_frame_size*2) {
            if (this->audio_ring_buf.read(this->vis_audio_data,
                    this->fft_frame_size*2) >= this->fft_frame_size*2) {

                for (int i = 0; i < this->fft_frame_size; i++) {
                    this->vis_audio_data[i] = (this->vis_audio_data[i*2] +
                        this->vis_audio_data[i*2 + 1]) * 0.5f;
                }
                ...
                // scale spectrum data
                for (int i = 0; i < this->spectrum_bar_count; i++) {
                    this->cbar_heights[i] = 46.0f + this->cbar_heights[i];
                    if (this->cbar_heights[i] <= 0.0f) {
                        this->cbar_heights[i] = 0.01f;
                    } else {
                        this->cbar_heights[i] /= 46.0f;
                    }
                }
            }
        }
